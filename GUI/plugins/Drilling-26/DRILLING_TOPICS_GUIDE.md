...
| **Dual Motor Control** | `/drilling/motors_cmd` | `std_msgs/msg/Int32MultiArray` | `drilling_motors_cmd` | `[motor1_state, motor2_state]`<br>• `1` = Clockwise (CW)<br>• `0` = OFF<br>• `2` = Counter-Clockwise (CCW) | **Independent Dual Motor Control**: Triggered by the 3-state button groups (**CW**, **OFF**, **CCW**) for **Motor 1** and **Motor 2**. Allows fine-grained rotational control of individual drilling motors. |
