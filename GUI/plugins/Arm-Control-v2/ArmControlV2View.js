...
(function () {
    ...
    class ArmControlV2View {
        constructor(container, openmct, wsUrl = "ws://localhost:8080") {
            ...
            // FK state
            this.jointNames = ['joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'joint6', 'joint7'];
            this.jointValues = {
            ...
            this.jointRanges = {
                ...
                joint2: { min: -180, max: 180,  label: 'Joint 2 (°)' },
                joint3: { min: -180, max: 180,  label: 'Joint 3 (°)' },
                joint4: { min: -180, max: 180,  label: 'Joint 4 (°)' },
                joint5: { min: -180, max: 180,  label: 'Joint 5 (°)' },
                joint6: { min: -180, max: 180,  label: 'Joint 6 (°)' },
                joint7: { min: 0,    max: 1,    label: 'Gripper (0-1)' }
            };
            ...
            // Presets
            this.fkPresets = {
                home: { joint1: 0, joint2: 0,   joint3: 0,  joint4: 0, joint5: 0,  joint6: 0, joint7: 0 },
                rest: { joint1: 0, joint2: -45,  joint3: 90, joint4: 0, joint5: 45, joint6: 0, joint7: 0 }
            };
        ...
        sendLockOrientation() {
            ...
            console.log("[ArmControlV2View] Lock orientation:", this.lockOrientation ? 'ON' : 'OFF');
        }
        ...
        applyFKPreset(type) {
            ...
            this.jointNames.forEach(joint => {
                ...
                if (this.fkSliders[joint]) {
                    ...
                    // Dispatch the input event to trigger slider.oninput and send the network update
                    this.fkSliders[joint].dispatchEvent(new Event('input', { bubbles: true }));
                }
            });
            
            // Explicitly send the update over the WebSocket just to be completely sure
            this.sendUpdate();
        }
